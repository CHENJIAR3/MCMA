...
def paddingfor16(ecg):
    ...
    return ecgnew
@tf.function(experimental_relax_shapes=True)
def test_ekgan(model,datapath,output_num=2,numlead=1):
    rhr=np.zeros((4,1))
    fhr=np.zeros((4,1))
    maehr=0
    ...
    for ecgpath in ecgpaths:
        print(ecgpath)
        ecg12 = sio.loadmat(ecgpath)['ECG'][0][0][2]

        # num+=ecg12.shape[0]
        ecg12 = tf.transpose(ecg12)
        ecg12 = tf.expand_dims(ecg12,axis=0)
...
@tf.function(experimental_relax_shapes=True)
def test_unet(model,datapath,numlead=1):
    # 输入为一导联
    maehr=0
    rhr=np.zeros((4,1))
    fhr=np.zeros((4,1))
...
def get_hrmetric(hr1):
    ...
    return rhr
@tf.function(experimental_relax_shapes=True)
def test_ae(model,datapath,numlead=12):
    # 输入为一导联
    ...
    for ecgpath in ecgpaths:
        ...
        ecg12_new = tf.concat([ecg12, tf.zeros_like(ecg12)[:, -padding_len:, :]], axis=1)
        ecg12_new = tf.cast(tf.reshape(ecg12_new, shape=(-1, 1024, 12)),dtype=tf.float32)
...
def final_get(a):
    return a[:-1,:]/a[-1,:][None,:]
def write2excel(rhr,fhr,maehr,excel_path):
    # print(CC_test)
...
if __name__=='__main__':
    ...
    datapath = '/data/0shared/chenjiarong/CPSC2018/'
    args.testmodel= 'ae'
    ...
    if args.testmodel == 'ae':
        ...
        for [anylead, padding] in [[1, 'zeros']]:
            ...
            maehr,rhr, fhr = test_ae(model=model,datapath=datapath)
            excel_path = resultpath + 'cpsc2018_hr_0814.xlsx'
            if not os.path.exists(resultpath):
                os.mkdir(resultpath)
            write2excel(rhr, fhr, maehr,excel_path)

    # if args.testmodel=='unet':
    modelpath='../MAUNet'
    model1 = tf.keras.models.load_model(modelpath)
    maehru,rhru,fhru=test_unet(model=model1,datapath=datapath,numlead=2)
    # excel_path = resultpath + 'MAUNet_hr.xlsx'
    # if args.testmodel=='ekgan':
    modelpath='../EKGAN/inference_generator'
    model1 = tf.keras.models.load_model(modelpath)
    maehre,rhre, fhre=test_ekgan(model=model1,datapath=datapath)
        # excel_path = resultpath + 'EKGAN_hr.xlsx'
    # if args.testmodel=='megan':
    modelpath = '../MEGAN/generator'
    model1 = tf.keras.models.load_model(modelpath)
    maehrm,rhrm,fhrm = test_unet(model=model1,datapath=datapath)
        # excel_path = resultpath + 'MEGAN_hr.xlsx'
    # if args.testmodel=='cgan':
    modelpath = '../CGAN/generator'
    model1 = tf.keras.models.load_model(modelpath)
    maehrc,rhrc,fhrc = test_unet(model=model1,datapath=datapath)
        # excel_path = resultpath + 'CGAN_hr_0409.xlsx'

    excel_path=resultpath+"compared_hr_0410.xlsx"
    if not os.path.exists(resultpath):
        os.mkdir(resultpath)
    maehr=np.concatenate([maehru,maehre,maehrm,maehrc],axis=1)
    ...
    with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
        ...
        df2.to_excel(writer, sheet_name='fake_heartrate', index=True)
        df3.to_excel(writer, sheet_name='MAE_HR', index=True)
    # write2excel(rhr,fhr, excel_path)
